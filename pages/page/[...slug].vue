<template>
  <div class="bg-base-100 text-base-content">
    ...
    <div v-if="page" class="hero min-h-[40vh] relative">
      ...
      <div class="absolute inset-0" :style="page?.image ? 'background: linear-gradient(135deg, rgba(239, 86, 164, 0.4) 0%, rgba(74, 144, 226, 0.4) 100%)' : 'background: linear-gradient(135deg, #EF56A4 0%, #4A90E2 100%)'"></div>
      <!-- Container-constrained background image -->
      <div v-if="page?.image" class="absolute inset-0 container mx-auto">
        <div class="h-full" :style="`background: url('${page.image}'); background-size: cover; background-position: center;`"></div>
      </div>
      <div class="hero-content text-center relative z-10">
        <div class="backdrop-blur-sm bg-white/10 p-8 rounded-3xl shadow-2xl border border-white/20 max-w-3xl">
          ...
          <div v-if="page?.subtitle" class="text-xl text-white" style="text-shadow: 0 0 5px rgba(0,0,0,0.7);">
            {{ page.subtitle }}
          </div>
        </div>
      </div>
    ...
    <div class="py-16 container mx-auto px-4 max-w-4xl">
      <article v-if="page" class="card bg-base-200 rounded-2xl border border-base-300 p-8 mb-8">
